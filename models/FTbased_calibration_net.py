...
class CN(nn.Module):
    ...
    def forward(self, logits, image_emb, text_emb):
        ...
        # print(torch.softmax(logits * t, dim=1).max(1)[0]) # 可能是softmax的维度太大了，不应该是softmax再加softmax,应该是logits在是softmax
        # exit()
        #
        # return confidence*t
...
class CalibrationNet():
    ...
    def train_cn(self, train_data, val_data):
        ...
        self.FT_model.classes = self.classes
        ...
        # Get prompts
        with torch.no_grad():
            text_embs = self.FT_model.model(self.FT_model.classes)
            # text_features = text_features / text_features.norm(dim=-1, keepdim=True)
            text_embs = text_embs.float()
            text_features = text_embs / text_embs.norm(dim=-1, keepdim=True)
        log.info(f"TEXT FEATURES SHAPE: {text_embs.size()}")
        ...
        for i, (img, _, _, label, img_path) in enumerate(train_loader):  # 这里batch_size = len(train_data)也就是只有一次循环
            # Calculate the output probability of CLIP
            img = img.to(self.device)
            with torch.no_grad():
                image_embs = self.clip_model.encode_image(img).float()
            ...
            with torch.no_grad():
                ...
                p_text_embs = text_embs[torch.argmax(image_embs @ text_embs.t(), dim=-1)].float() # 对应预测的Test Embs


        for i, (val_img, _, _, val_label, img_path) in enumerate(val_loader):  # 这里batch_size = len(train_data)也就是只有一次循环
            # Calculate the output probability of CLIP
            val_img = val_img.to(self.device)
            with torch.no_grad():
                val_image_embs = self.clip_model.encode_image(val_img).float()
        ...
        self.optimizer = torch.optim.Adam( # 试一试 Adam， SGD
        ...
        for epoch in range(self.config.META_EPOCHS):
            ...
            #         print(name, param.grad)  # 检查是否为 None
