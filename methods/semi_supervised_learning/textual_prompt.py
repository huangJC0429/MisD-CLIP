...
class TextualPrompt(TrainingStrategy):
    ...
    def _train_epoch(
        ...
        if torch.cuda.is_available():
            ...
            print(f"[self._train_epoch] Number of prompts: {len(self.model.classes)}")
    ...
    def test_predictions(self, data, standard_zsl=False):
        ...
        # if self.config.source_model != 'CLIP':
        #     calibration_model = torch.load("./saved_model/" + self.config.DATASET_NAME + "_16" + ".pth")
        ...
        for img, _, _, img_path in test_loader:
            ...
            # if self.config.source_model != 'CLIP':  # use our model to calibrate logits.
            #     calibration_model.eval()
            #     # print(logits.shape)
            #     # print(image_features.shape) # torch.Size([16, 512])
            #     # print(text_features.shape) # torch.Size([47, 512])
            #     test_p_text_embs = text_features[torch.argmax(image_features @ text_features.t(), dim=-1)].float()
            #     logits = calibration_model(logits.float(), image_features.float(), test_p_text_embs)
            ...
            logits_all += [logits.float()]

        test_logits = torch.cat(logits_all, axis=0).detach().to('cpu')  # the original logits outputed by CLIP
        ...
        # recoding the logits for calculating metrics
        # df_predictions['test_logits'] = test_logits

        return df_predictions, test_logits
    ...
    def clip_test_predictions(self, data):
        """
        ...
        #     print(i[0].shape) #[16, 3, 224, 224]
        #     print(i[1].shape) #[16, 3, 224, 224]
        #     print(i[2].shape) #[16, 3, 224, 224]
        #     print(i[3])  # [16] 标签
        #     print(i[4])  # img_file
        #     print(i[0] == i[1])  # True
        #     exit()

        # print(text.shape) # 47, 77 47个类别

        # Iterate through data loader 下一步看看计算logits
        # for img, _, _, labels, img_path in test_loader: #前3个都是相同的 train的数据集有labels，test的没有
        for img, _, _, img_path in test_loader:
            with torch.no_grad():
                ...
                predictions += [self.classes[i] for i in idx_preds]
                images += [i for i in img_path]
                prob_preds += [logits_per_image]  # probs, logits_per_image
